...
console.log("=== ЗАПУСК ВАЛИДАЦИОННЫХ ТЕСТОВ СЕМАНТИКИ (CYCLE 7 — TASKS 31 & 32) ===");
...
// Validate the new Level 4 nodes presence and structure
const targetIds = [
  'l4_11_deploy_strategy',
  'l4_12_shadcn_revolution'
];
...
targetIds.forEach(id => {
  ...
  if (node.level !== 'L4') {
    console.error(`❌ Ошибка: Узел ${id} имеет неверный уровень: ${node.level} (ожидался L4)`);
    failed = true;
  }
...
if (failed) {
  ...
  console.log("⭐️ ВСЕ СЕМАНТИЧЕСКИЕ ТЕСТЫ CYCLE 7 (TASKS 31 & 32) УСПЕШНО ПРОЙДЕНЫ!");
  process.exit(0);
}
