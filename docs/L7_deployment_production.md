...
## 🍏 Раздел 13: Особенности Apple Developer Program, Тестирование и Git-хуки

Развертывание мобильных и кроссплатформенных приложений накладывает дополнительные требования к инфраструктуре и процессам доставки.

### 13.1. Apple Developer Program: $99, Сертификаты и Ограничения
Если ваше приложение дистрибутируется на платформе iOS/macOS, вам не обойтись без платной подписки Apple:
- **Стоимость подписки**: $99 ежегодно. При неоплате приложения скрываются из App Store.
- **Типы аккаунтов**:
  - *Individual*: Оформляется на физлицо мгновенно. Имя разработчика в App Store будет вашим личным именем.
  - *Organization*: Требует наличия юридического лица и международного номера **D-U-N-S** (получение может занять от 3 дней до 2 недель). Позволяет добавлять других разработчиков в команду.
- **Подписи и Сертификаты (Provisioning & Certificates)**:
  - **Development Certificate**: Для запуска приложения на локальных тестовых устройствах.
  - **Distribution Certificate**: Для отправки приложения в TestFlight и публикации в App Store.
  - **Provisioning Profile**: Связующее звено между вашим Apple ID, App ID (Bundle ID) и зарегистрированными UDID тестовых девайсов.
- **App Store Connect**:
  - **TestFlight**: Бесплатная встроенная песочница для внешнего и внутреннего тестирования (до 10,000 внешних тестировщиков). Идеально для бета-тестов перед основным релизом.
  - **Рецензирование (App Review)**: Каждая сборка проверяется вручную модераторами Apple по регламенту App Store Review Guidelines (обычно занимает 24–48 часов). Для соло-разработчика критично следовать правилу 2.1 (Performance & Stability), иначе приложение получит мгновенный Reject.

---

### 13.2. Изоляция тестов: Unit vs Integration Tests SOP
Чтобы тесты проходили мгновенно на компьютере разработчика и были надежными в CI-среде, разделяйте их на два изолированных уровня:

1. **Unit-тесты (Модульные)**:
   - Должны выполняться без сетевого доступа и без подключения к реальной базе данных.
   - Базы данных, API Stripe, Brevo и т.д. должны быть полностью замоканы (например, через `vi.mock` в Vitest).
   - Скорость выполнения: менее 10 мс на один тест.
2. **Integration-тесты (Интеграционные)**:
   - Проверяют реальные сквозные запросы.
   - Для ускорения интеграционных тестов в локальном CI используйте встраиваемую файловую базу данных **SQLite** (или запускаемый в Docker контейнер PostgreSQL), которая сбрасывается и накатывает миграции заново перед каждым запуском тестов.
   - Никогда не направляйте интеграционные тесты на живую Production-базу данных.

#### Архитектура папок тестов:
```
src/
  ├── __tests__/
  │   ├── unit/
  │   │   ├── auth.test.ts      # Мокированная логика
  │   │   └── formatters.test.ts
  │   └── integration/
  │       ├── payment.test.ts   # Тест с тестовой БД
  │       └── user-flow.test.ts
```

---

### 13.3. Git Hooks: Автоматический контроль качества перед коммитом
Git-хуки позволяют запретить коммит или пуш кода, который содержит синтаксические ошибки, не проходит линтинг или ломает тесты. Это страховка от отправки "грязного" кода в репозиторий.

#### 1. Вариант через Husky & lint-staged (современный JS-стандарт):
...
Настройка `.husky/pre-push` (запуск тестов перед отправкой на GitHub):
```bash
#!/bin/sh
. "$(dirname "$0")/_/husky.sh"
...
#### 2. Вариант на чистом Bash (универсальный для любого стека):
Создайте файл `.git/hooks/pre-commit` (и сделайте его исполняемым `chmod +x`):
```bash
#!/bin/sh

echo "==== [Git Hook] Running Typecheck & Lint ===="
npx tsc --noEmit && npx eslint . --ext .ts,.tsx

if [ $? -ne 0 ]; then
  echo "❌ [Git Hook] Typecheck or Lint failed. Commit aborted!"
  exit 1
fi

echo "✅ [Git Hook] Code quality verified. Proceeding..."
```
