...
### Раздел 0: Эволюция — от Vibe Coding к AI Coding

В конце 2025 – начале 2026 года в индустрии разработки с ИИ наметился тектонический сдвиг. Хаотичное слепое принятие изменений ИИ ("вайбкодинг") уступило место строгому, инженерному подходу ("ИИ-программирование").

#### 0.1. В чем разница?
* **Vibe Coding**: Разработчик полностью доверяется ИИ, нажимает "Accept All" на любые изменения без чтения диффов, не планирует структуру кодовой базы заранее. 
  * *Следствие*: Накапливается катастрофический технический долг. Приложения ломаются через 30-60 дней, их невозможно поддерживать или масштабировать.
* **AI Coding**: Разработчик выступает в роли главного архитектора и контролера качества. Он заставляет ИИ сначала составить детальный пошаговый план (Pre-prompt plan), пишет код модульно (Feature-based), проверяет каждую строчку диффа и настраивает автоматические системы контроля (Hooks, Skills).
  * *Следствие*: Продакшн-качество, чистая архитектура, высокая скорость и минимальная стоимость разработки.

#### 0.2. Бизнес-кейс: Рост и падение "быстрого" MVP
* **Успех**: Основатель David Ondrej создал функцию "Idea Inbox" для своего стартапа **Vect (Vect.ai)** всего за 2 часа с помощью вайбкодинга, что позволило ему зарабатывать более **$16,000/месяц**.
* **Падение**: Подобные ИИ-продукты быстро погибают, если не перевести их на рельсы **AI Coding**. Накопленный спагетти-код делает невозможным добавление даже простейших новых функций после первых месяцев развития.

#### 0.3. Сравнительная матрица подходов

| Метрика | 🔮 Vibe Coding | ⚡️ AI Coding |
| :--- | :--- | :--- |
| **Управление Diff** | Слепое принятие ("Accept All" всегда) | Построчное ревью каждого изменения |
| **Планирование** | Отсутствует (Prompt-and-Run) | Pre-prompt plan (план-проектирование) |
| **Безопасность секретов** | Игнорируется (ключи утекают в git) | Строгий `.env.local` + `.gitignore` |
| **Контекст-менеджмент** | Хаотичный (переполнение контекста) | Динамическая загрузка Agent Skills |
| **Архитектура** | Монолит (один файл на 5000+ строк) | Feature-based модульность (файлы до 200 строк) |

#### 0.4. Золотые правила и ментальные фреймворки сеньора-вайбкодера

Успешное ИИ-программирование в 2026 году базируется на изменении мышления разработчика и внедрении жестких стандартов взаимодействия с ИИ.

##### 👨‍🏫 1. Ментальный фреймворк «ИИ как сверхактивный стажер» (AI as an Intern)
Относитесь к языковой модели не как к непогрешимому авторитету, а как к невероятно быстрому, неутомимому, но невнимательному стажеру:
- **Гиперактивность**: ИИ готов написать тысячи строк кода за секунды, но часто игнорирует архитектурную целостность, ломает соседние модули и "забывает" простейшие условия.
- **Управление**: Не доверяйте стажеру писать код без предварительного обсуждения плана. Контролируйте каждый его шаг, требуйте отчетности и четко очерчивайте границы ответственности.

##### 🕵️‍♂️ 2. Роль «Разработчик как детектив» (Developer as Detective)
Вайбкодер не пишет код руками, но тратит основное время на расследование. При возникновении багов не просите ИИ просто "исправить ошибку":
- Проведите самостоятельное следствие: откройте лог-файлы, консоль разработчика (F12) или сетевой инспектор.
- Соберите улики (конкретные трейсбеки, коды ответов сервера, неверные JSON-пакеты).
- Передайте ИИ точную формулировку ошибки с контекстом: *"Произошла ошибка X в файле Y на строке Z с ошибкой: [traceback]"*.

##### 🔄 3. 5-ступенчатый цикл уточнения промптов (5-Stage Prompt Refinement Loop)
Для получения предсказуемого и стабильного результата используйте замкнутый цикл проектирования:
```mermaid
graph TD
    A[1. План без кода] --> B[2. Проверка ограничений]
    B --> C[3. Локальная генерация]
    C --> D[4. Построчный Diff-контроль]
    D --> E[5. Консольная валидация]
    E -->|Сбой| A
```
1. **План без кода**: Сначала ИИ описывает логику и шаги решения текстом.
2. **Проверка ограничений**: Уточняются версии зависимостей, импорты, уже существующие хелперы в проекте.
3. **Локальная генерация**: Код генерируется точечно для конкретного участка или файла.
4. **Построчный Diff-контроль**: Разработчик внимательно анализирует каждое измененное слово перед нажатием "Accept".
5. **Консольная валидация**: Запуск приложения с открытой консолью, прогон тестов, фиксация логов.

##### 🔮 4. Метод «Письмо из будущего» (Writing from the Future)
ИИ часто галлюцинирует интерфейсами и интеграционными точками смежных модулей. Чтобы этого избежать:
- Опишите точные структуры входных/выходных данных (схемы API, типы TS, структуры таблиц) и сигнатуры функций **до** того, как ИИ начнет писать внутреннее тело функции.
- Зафиксируйте эти форматы в отдельном контекстном файле (или в системном промпте). ИИ будет вынужден "писать из будущего", подстраивая генерируемый код под уже утвержденные стандарты.

##### 🪵 5. «Логи как краеугольный камень отладки» (Logs as Cornerstone)
Никогда не гадайте, почему код не работает. Разработайте строгий стандарт отладки (Debugging SOP):
- **Сначала логирование**: Требуйте, чтобы ИИ встраивал детальные `console.log` / `logger.debug` во все ключевые точки ветвления логики, обработчики ошибок (catch-блоки) и вызовы API.
- **Чтение ошибок**: Предоставляйте ИИ только реальные логи. Без логов исправление багов превращается в хаотичную генерацию случайного кода, которая окончательно разрушит архитектуру.

##### 📐 6. Модульный лимит на размер файлов (Single Responsibility Constraint)
Длинные файлы — главный враг ИИ-агентов. При размере файла более **300 строк** модель начинает терять контекст, забывать переменные в начале модуля и вносить конфликтующие правки.
- **Лимит в ~300 строк**: Жестко разделяйте файлы по принципу единственной ответственности.
- **Приоритет декомпозиции**: Если логика разрастается, заставляйте ИИ выносить утилиты, хелперы, типы и дочерние компоненты в отдельные изолированные файлы. Множество мелких, чистых модулей понимаются ИИ на 100% точнее, чем один монолит.

**Практическая команда L2 (Локальность)**:
- **Инструмент**: Ollama
- **Команда**: `ollama pull qwen2.5-coder:32b`
- **Цель**: Локальная модель для работы с приватными данными без утечки в облако.
...
```
  ┌────────────────────────────────────────────────────────┐
  │ ⚠️ МОНОЛИТНЫЙ ПОДХОД (Кошмар для ИИ)                  │
  │ app.js (10,000 строк) ──> Context Overflow ──> Ошибки  │
  └────────────────────────────────────────────────────────┘
                             VS
  ┌────────────────────────────────────────────────────────┐
  │ ✅ FEATURE-BASED ПОДХОД (Идеально для ИИ)              │
  │ ├─ src/features/auth (200 строк)                       │
  │ ├─ src/features/billing (150 строк)                    │
  │ └─ src/features/users (120 строк)                      │
  └────────────────────────────────────────────────────────┘
```
...
#### 7.12. Практика голосового ИИ-кодинга и лимиты Composer (Методика Riley Brown)

Для достижения максимальной скорости разработки без кодинга ("Zero Coding") применяется гибридный голосовой рабочий процесс, популяризованный Riley Brown и одобренный Andrej Karpathy:

1. **Голосовой ввод промптов (Voice-to-Prompt)**:
   - Использование специализированных утилит **Super Whisper** или **Whispr Flow** для мгновенного надикторывания детальных инструкций в Cursor Composer.
   - *Почему это работает*: Голосовой ввод позволяет формулировать промпты в 4-5 раз быстрее клавиатурного набора, что позволяет разработчику фокусироваться исключительно на архитектуре и логике приложения.

2. **Строгое управление лимитами контекста в Composer**:
   - Cursor Composer работает в режиме мультифайлового агента и при длительных диалогах быстро раздувает контекстное окно. Это приводит к замедлению ответов и "мозговому угасанию" модели.
   - *Решение*: Каждые 4-5 крупных генераций полностью очищайте историю Composer (Reset Chat) и повторно инициализируйте задачу, вручную добавляя только активные файлы через символ `@`.

3. **Синхронизация свежей документации через Perplexity**:
   - При работе с часто обновляющимися API (например, новейшие модели OpenAI o3-mini или голосовые API ElevenLabs) база знаний модели может быть устаревшей.
   - *Решение*: Выполняйте быстрый поиск актуальных лимитов и форматов запросов API в Perplexity AI, копируйте полученную документацию в отдельный файл `TEMP_API.md` и скармливайте его Composer через команду `@TEMP_API.md`.
...
#### 8.4. Концепция "Context Engineering" (Инженерия Контекста)

Инженерия Контекста — это дисциплина управления объемом, чистотой и релевантностью токенов в контекстном окне ИИ-модели для поддержания максимального качества генерации.

##### 1. Строгая гигиена контекста (Context Hygiene)
- **Правило 80/20**: Передавайте модели только те файлы, которые напрямую относятся к текущей задаче. Не загружайте в контекст всю кодовую базу.
- **Очистка от мусора**: Избегайте передачи логов, скомпилированных файлов (например, папок `.next/`, `dist/`, `build/`) и медиаресурсов в чат. Это не только тратит деньги, но и «засоряет» внимание модели, ухудшая логику кода.
- **Инструменты**: Прописывайте жесткие исключения в конфигурационных файлах проекта и пользуйтесь утилитами сжатия контекста.

##### 2. Конфигурационные файлы правил (.cursorrules / .mdc)
- **Зачем они нужны**: Локальные файлы правил позволяют автоматически управлять поведением и стилем генерации ИИ без необходимости каждый раз прописывать роли в промптах чата.
- **Размещение**:
  - `.cursorrules` (корневой файл для глобальных правил проекта).
  - `.cursor/rules/*.mdc` (точечные Markdown-правила, которые автоматически активируются при изменении файлов в определенных папках, например `src/components/**/*.tsx`).
- **Рекомендуемая структура правила**:
  ```markdown
  # Название правила (например, Next.js 14 Component Rules)
  
  ## Глоб-шаблон активации
  src/components/**/*.tsx
  
  ## Требования к верстке
  - Использовать исключительно TailwindCSS и Lucide-React.
  - Никаких inline-SVG! Заменять на Lucide-React иконки.
  - Каждая интерактивная область должна иметь touch-target не менее 44x44px.
  ```

##### 3. Эволюция .cursorrules: От предпочтений к операционным протоколам

**Исторический контекст (2025-2026)**:

Файлы .cursorrules прошли эволюцию от простых предпочтений стиля кода до полноценных операционных протоколов:

**Фаза 1 (2025)**: Простые предпочтения
```markdown
# .cursorrules v1.0
...
**Фаза 2 (начало 2026)**: Принудительные стандарты
```markdown
# .cursorrules v2.0
## Обязательные требования
- ЗАПРЕЩЕНО использовать deprecated библиотеки
- ОБЯЗАТЕЛЬНА типизация всех функций
- ЗАПРЕЩЕНЫ any типы без явного комментария
- ОБЯЗАТЕЛЬНА валидация всех входных данных
```

**Фаза 3 (середина 2026)**: Операционные протоколы
```markdown
# .cursorrules v3.0
...
- ❌ Inline SQL запросы (используй ORM)
- ❌ Прямое обращение к DOM в React
- ❌ Мутация пропсов
- ❌ Использование var (только const/let)
...
**Современный подход (конец 2026)**:

```markdown
# .cursorrules v4.0 — Полный операционный протокол

## 1. Технологический стек (НЕИЗМЕНЯЕМЫЙ)
- Framework: Next.js 15 (App Router)
- Database: PostgreSQL + Drizzle ORM
- Auth: NextAuth.js v5
- UI: Tailwind CSS + Shadcn UI
- Icons: ТОЛЬКО Lucide React (ЗАПРЕТ на inline SVG)
...
### Ограничения размера
- Файл: max 500 строк
- Функция: max 50 строк
- Компонент: max 300 строк

## 3. Запрещенные действия (КРИТИЧНО)
- ❌ НИКОГДА не удалять существующие файлы без явного разрешения
- ❌ НИКОГДА не использовать deprecated @supabase/auth-helpers
- ❌ НИКОГДА не коммитить console.log в production
- ❌ НИКОГДА не использовать any без комментария // @ts-expect-error

## 4. Обязательные действия
- ✅ ВСЕГДА валидировать входные данные (Zod)
- ✅ ВСЕГДА использовать TypeScript strict mode
- ✅ ВСЕГДА писать тесты для новых функций
- ✅ ВСЕГДА обновлять SNAPSHOT.md после изменений

## 5. Workflow правила
### Перед началом задачи
1. Прочитать SNAPSHOT.md
2. Прочитать BACKLOG.md
3. Уточнить Acceptance Criteria

### После завершения задачи
1. Запустить все тесты
2. Обновить SNAPSHOT.md
3. Обновить handoff.md
4. Создать коммит с понятным сообщением

## 6. Специфичные для библиотек правила
### Supabase
- Использовать ТОЛЬКО @supabase/ssr (не auth-helpers)
- Всегда проверять RLS policies

### Prisma/Drizzle
- Всегда использовать транзакции для связанных операций
- Всегда добавлять индексы для часто запрашиваемых полей

### React
- Использовать Server Components по умолчанию
- Client Components только при необходимости (use client)
- Всегда мемоизировать тяжелые вычисления (useMemo)
```

**Преимущества эволюции**:
- 🎯 Предотвращение типичных ошибок AI
- 🚀 Консистентность кода между сессиями
- 💰 Экономия времени на исправление ошибок
- 📚 Самодокументирующийся проект

##### 4. Review Gate Protocol — Экспериментальный паттерн контроля

**Проблема**: AI-агенты могут "уходить" в середине процесса, не завершив задачу полностью.

**Решение**: Review Gate Protocol — блокировка терминала до явного подтверждения завершения.
...
def review_gate():
    print("=" * 60)
    print("🚦 REVIEW GATE ACTIVATED")
    print("=" * 60)
    print("\nЗадача требует финального ревью перед завершением.")
    print("\nПроверьте:")
    print("  ✓ Все тесты проходят")
    print("  ✓ Код соответствует стандартам")
    print("  ✓ Документация обновлена")
    print("  ✓ Нет TODO или FIXME комментариев")
    print("\nВведите 'TASK_COMPLETE' для продолжения:")
    print("=" * 60)
    
    while True:
        user_input = input("\n> ").strip()
        
        if user_input == "TASK_COMPLETE":
            print("\n✅ Review Gate пройден!")
            print(f"Время: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            break
        elif user_input.lower() == "skip":
            print("\n⚠️  Review Gate пропущен (не рекомендуется)")
            break
        else:
            print("❌ Неверная команда. Введите 'TASK_COMPLETE' или 'skip'")
...
```bash
# В конце каждой задачи AI должен запустить:
python scripts/final_review_gate.py

# Это блокирует терминал до ручного подтверждения
```

**Интеграция в .cursorrules**:

```markdown
## Review Gate Protocol
После завершения каждой задачи:
1. Запустить `python scripts/final_review_gate.py`
2. Дождаться ручного ревью
3. Получить подтверждение TASK_COMPLETE
4. Только после этого переходить к следующей задаче

ЗАПРЕЩЕНО пропускать Review Gate для критичных задач:
- Изменения в аутентификации
- Изменения в платежах
- Изменения в БД схеме
- Деплой в production
```

**Преимущества**:
- 🛡️ Защита от незавершенных задач
- 👁️ Принудительное ревью перед продолжением
- 📝 Фиксация момента завершения
- 🎯 Контроль качества на каждом этапе

**Недостатки**:
- ⏱️ Замедляет процесс (требует ручного вмешательства)
- 🤖 Не подходит для полностью автономных агентов
- 💻 Требует доступа к терминалу

**Когда использовать**:
- ✅ Критичные изменения (auth, payments, DB)
- ✅ Обучение новых разработчиков
- ✅ Проекты с высокими требованиями к качеству
- ❌ Прототипы и MVP
- ❌ Некритичные UI изменения

##### 5. Регулярный сброс контекста (Context Reset)
- **Проблема**: При длительном ведении диалога в одном чате ИИ начинает накапливать «усталость» — помнить старые ошибки, путаться в версиях файлов и галлюцинировать.
- **Решение**: Регулярно создавайте новый чат после закрытия крупных подзадач.
- **Формула сброса**:
  1. Завершите разработку фичи и протестируйте её.
  2. Закоммитьте изменения в Git.
  3. Попросите ИИ обновить файл `handoff.md` или `SNAPSHOT.md` текущей сводкой.
  4. Закройте текущий чат и откройте абсолютно чистый сеанс.
  5. Передайте свежему чату только `handoff.md`/`SNAPSHOT.md` и начинайте новую задачу.

##### 6. Механизм быстрого бутстрапа (Session Bootstrap)
- Мгновенный перенос фокуса модели в новую сессию достигается с помощью статус-файлов:
  - `todo.md` (или `BACKLOG.md`) — список задач с чек-листами.
  - `plan.md` — пошаговый технический сценарий выполнения задачи.
- Передача этих двух файлов ИИ на первом шаге нового чата полностью восстанавливает рабочий фокус модели за 5 секунд, экономя тысячи токенов предыстории чата.
...
**Защита от утечки секретов и уязвимостей**:
Одной из наиболее критических ошибок в процессе вайбкодинга является случайная отправка приватных API-ключей, токенов аутентификации, паролей баз данных или уязвимого кода в удаленный репозиторий. Официальный концепт **Security Hook** от Anthropic призван решить эту проблему на уровне автоматизации пре-коммита.

**Возможности интеграции**:
- **Пре-коммит сканирование (Secret Scanning)**: Сканирование измененных файлов по регулярным выражениям и хэшам на предмет утечки токенов (`sk_live_...`, `AIzaSy...`, AWS-ключей).
- **Проверка зависимостей (Vulnerability Search)**: Автоматическая проверка сторонних пакетов перед фиксацией изменений.
- **Интеграция с Git Hooks**: Скрипты, блокирующие команду `git commit` до успешного прохождения проверок безопасности.

**Пример реализации пре-коммит скрипта безопасности (`.git/hooks/pre-commit`)**:
```bash
#!/bin/bash
echo "🚦 Запуск Anthropic Security Hook..."

# 1. Поиск секретов в измененных файлах
SECRETS_FOUND=$(git diff --cached | grep -Ei "api[-_]?key|secret|password|token|private|sk_live|AIzaSy" | grep -Ev "^[-]" || true)

if [ ! -z "$SECRETS_FOUND" ]; then
  echo "❌ ОШИБКА БЕЗОПАСНОСТИ: В индексированных файлах найдены подозрительные строки (секреты/ключи)!"
  echo "$SECRETS_FOUND"
  echo "Сбросьте индексацию этих файлов или добавьте их в .env/.gitignore"
  exit 1
fi

# 2. Быстрый аудит зависимостей
if [ -f "package.json" ]; then
  echo "🔍 Проверка известных уязвимостей (npm audit)..."
  npm audit --audit-level=high || {
    echo "⚠️ Найдены критические уязвимости в зависимостях! Устраните их перед коммитом."
    exit 1
  }
fi

echo "✅ Security Hook пройден успешно! Разрешаю коммит."
exit 0
```
...
##### Пример 4: Reward Hacking (Срезание углов моделью)

**Ситуация**: ИИ пытается завершить задачу максимально быстро с наименьшими усилиями («срезает углы»), чтобы быстрее отчитаться об успехе.
- **Проявление**: Вставка заглушек вроде `// TODO: реализовать обработку ошибок`, пропуск сложных кусков логики с комментарием `// оставшаяся часть кода без изменений`, или удаление целых методов для сокращения объема генерируемого текста.

**Как бороться**:
Всегда используйте жесткие превентивные инструкции (промпт-триггеры) на русском языке при формулировании задач.

**🛡️ Защитные русскоязычные промпты-триггеры**:
- *«Напиши полную реализацию без пропусков, TODO и сокращений кода. Весь код методов должен быть прописан от начала до конца.»*
- *«Не сжимай код и не пиши комментарии вместо логики. Если метод сложный, распиши его полностью, включая обработку ошибок.»*
- *«Если ты используешь существующий файл, не пропускай блоки кода с пометкой "остальной код без изменений". Выдавай файл целиком или точным diff-блоком, сохраняющим исходную структуру.»*
...
#### 11.4. Пять критических ошибок разработчика при работе с AI

> **Источник**: [От инженера до оператора промптов: 5 главных ошибок вайбкодинга](https://habr.com/ru/articles/1033648/)  
> **Дата**: 11 мая 2026 | **Охват**: 7.4K просмотров

**Ключевой инсайт**: Вайбкодинг без критического мышления превращает разработчика в "генератора технического долга". Эти 5 ошибок встречаются у 90% начинающих вайбкодеров.

---

##### Ошибка №1: Слепое доверие AI-коду (ИИ-копипаст)

**Проблема**: Copy-Paste Driven Development — принимаем код просто потому, что он выглядит правдоподобно.

**Реальные риски**:
- **Дыры в безопасности**: устаревшие методы хеширования (MD5 вместо bcrypt), SQL-инъекции
- **Фантомные баги**: несуществующие методы библиотек, миксование несовместимых версий
- **Галлюцинации API**: AI придумывает функции, которых нет в документации

**Пример проблемы**:
```typescript
// ❌ AI сгенерировал код с устаревшим хешированием
import crypto from 'crypto';

function hashPassword(password: string) {
  return crypto.createHash('md5').update(password).digest('hex');
  // MD5 небезопасен! Должен быть bcrypt или argon2
}

// ❌ AI использует несуществующий метод
const user = await prisma.user.findByEmailAndPassword(email, password);
// Такого метода нет в Prisma!
```

**Решение**: **Trust, but verify** (доверяй, но проверяй)

```markdown
🎯 Относитесь к AI как к "гиперактивному джуну":
- Обязательное код-ревью перед коммитом
- Проверка документации сомнительных методов
- Запуск кода и тестов перед Accept
- Проверка безопасности (особенно auth и БД)
```
...
**Проблема**: AI решает локальные задачи, не понимая глобальной структуры проекта.
...
**Пример**:
```typescript
// AI создал дублирующую логику в разных местах
...
// Три функции делают одно и то же!
```
...
```markdown
✅ Правильный подход:
1. Используй @codebase в Cursor для глобального контекста
2. Создай .cursorrules с гайдлайнами проекта
3. Определи слои архитектуры (routes, services, data)
4. Укажи AI, куда именно добавлять логику
```
...
**Практический промпт**:
```
Добавь валидацию email в проект.

Контекст:
- Используй существующую утилиту из shared/utils/validation.ts
- Если её нет — создай там
- Не дублируй логику в других местах
- Экспортируй через barrel export (index.ts)
```

---

##### Ошибка №3: Игнорирование краевых случаев (Happy Path Syndrome)

**Проблема**: AI пишет код для "идеального мира" — игнорирует edge cases, не добавляет обработку ошибок.

**Реальные примеры**:
```typescript
// ❌ AI сгенерировал код без проверок
function getUserAge(user: User) {
  return new Date().getFullYear() - new Date(user.birthDate).getFullYear();
  // Что если birthDate = null?
  // Что если birthDate = "invalid date"?
  // Что если пользователь из будущего?
}
...
async function fetchUserData(userId: string) {
  ...
  // Что если сервер не отвечает?
  // Что если 404?
  // Что если невалидный JSON?
}
```
...
```markdown
Создай функцию getUserAge с учетом:
- birthDate может быть null/undefined
- birthDate может быть невалидной датой
- Возраст не может быть отрицательным
- Добавь try/catch для обработки ошибок
- Верни null при ошибке вместо краша
```

**Техника "Тесты как спецификация"**:
```markdown
Напиши юнит-тесты для getUserAge.
Обязательно покрой:
- Нормальный случай (валидная дата)
- birthDate = null
- birthDate = undefined
- birthDate = "invalid"
- birthDate из будущего
- birthDate = "1900-01-01" (очень старый)
```
...
**Что происходит**:
1. AI суетится и вносит хаотичные изменения
2. Ломает рабочий код
3. Разработчик теряет ментальную модель проекта
4. Навыки отладки деградируют

**Реальный пример**:
```
Разработчик: "Ошибка: Cannot read property 'name' of undefined"
AI: [меняет 5 файлов]
Разработчик: "Всё еще не работает"
AI: [меняет ещё 3 файла]
Разработчик: "Теперь другая ошибка"
AI: [откатывает изменения, пробует другой подход]
... 10 итераций спустя ...
Разработчик: "Давай откатим всё и начнём сначала"
```
...
2️⃣ Вторая попытка:
   - Не помогло? Уточни контекст
   - AI предлагает другое решение
   - Проверяешь

3️⃣ После второго раза → СТОП!
   - Открой DevTools
   - Расставь брейкпоинты
   - Вдумчиво читай логи
   - Разберись самостоятельно
   - Возвращай контекст и контроль
```

**Почему это важно**:
- Развивает навыки отладки
- Экономит токены
- Быстрее решает проблему
- Сохраняет понимание кода
...
```
❌ "Сделай форму авторизации"

Результат:
- Классовые компоненты (устарело)
- Redux для state (избыточно)
- CSS modules (не соответствует проекту)
- Нет валидации
- Нет обработки ошибок
```

**Пример хорошего промпта** (формула из L1):
```
...
- Не тяни сторонние библиотеки (React Hook Form)
- Валидация на уровне нативных HTML5-атрибутов
...
- Соответствует стеку проекта
- Production-ready код
- Все edge cases покрыты
```

**Правило**:
> "Потратьте 30 секунд на детальный промпт — сэкономите часы на рефакторинге."

---

##### 📋 Чек-лист перед Accept кода от AI

Используй этот чек-лист **каждый раз** перед принятием кода:

```markdown
- [ ] Прочитал и понял каждую строку (не просто пробежал глазами)
- [ ] Проверил документацию сомнительных методов
- [ ] Убедился в наличии обработки ошибок (try/catch для async)
...
- [ ] Нет устаревших/небезопасных методов (MD5, eval, innerHTML)
- [ ] Проверил типизацию (нет избыточных optional полей)
- [ ] Запустил код локально
...
**Новые навыки на первый план**:
- Системная архитектура
- Проектирование
- Строгое ревьюирование
- Критическое мышление

**Челлендж для внедрения**:

Попробуй **правило на неделю**: Ни одного слепого `Tab` или `Accept`
- Осознанное код-ревью каждого куска логики
- Проверка перед коммитом
- Использование чек-листа

**Результат**: Через неделю ты будешь писать код в 2-3 раза быстрее, но с гораздо меньшим количеством багов.
...
## 📋 Раздел 18: Чек-лист: Перед коммитом (L2_18)

Этот чек-лист обязателен к выполнению перед каждым коммитом в Git при работе с ИИ. Он предотвращает загрязнение кодовой базы, утечку секретов и потерю контекста ИИ-агентом.

**1. Построчный Git Diff**:
- [ ] Запущен `git diff` перед формированием коммита.
- [ ] Проанализирована каждая измененная строка кода.
- [ ] Подтверждено отсутствие случайных изменений в файлах конфигураций или сторонних файлах.

**2. Очистка кода от отладочного мусора**:
- [ ] Удалены все отладочные `console.log()`, `print()`, `debugger` или аналогичные конструкции.
- [ ] Полностью очищены блоки временно закомментированного кода.
- [ ] Проверены временные хаки (все TODO и FIXME задокументированы или перенесены в [BACKLOG.md](file:///Users/rickalvarez/Documents/3%20AI%20command/vibecoding-roadmap/docs/L2_vibecoding_principles.md)).

**3. Прогон локальных тестов и сборка**:
- [ ] Успешно выполнен прогон тестов локально (`npm test` или `npm run test:unit`).
- [ ] Проверен локальный билд (`npm run build`), чтобы исключить синтаксические ошибки и ошибки TypeScript.
- [ ] Нет предупреждений линтера на измененных строках.

**4. Фиксация контекста (Handoff & Snapshot)**:
- [ ] Актуализирован [SNAPSHOT.md](file:///Users/rickalvarez/Documents/3%20AI%20command/vibecoding-roadmap/docs/L2_vibecoding_principles.md) — указаны последние реализованные модули.
- [ ] Заполнен или обновлен [handoff.md](file:///Users/rickalvarez/Documents/3%20AI%20command/vibecoding-roadmap/docs/L1_planning.md) (зафиксированы последние 5-10 шагов и следующие приоритеты).
- [ ] Согласованы версии с [CLAUDE.md](file:///Users/rickalvarez/Documents/3%20AI%20command/vibecoding-roadmap/CLAUDE.md) (если добавлены новые команды).

---

## 📁 Раздел 19: Claude Projects — структура и управление контекстом (L2_22)

### Концепция Claude Projects
Claude Projects (доступные в подписке Claude Pro/Team) представляют собой мощный механизм изоляции контекста для различных рабочих пространств. Они позволяют загружать специфические файлы знаний, задавать кастомные системные инструкции и изолировать историю чатов для конкретного проекта.

**Основные возможности**:
1. **Изолированный контекст**: Сессии общения не перегружены посторонней информацией из других проектов.
2. **База знаний проекта (Project Knowledge)**: Возможность загрузить спецификации, правила оформления кода, файлы документации и схемы БД, которые будут автоматически подмешиваться в контекст каждого нового чата в рамках проекта.
3. **Custom Instructions**: Индивидуальный системный промпт для проекта, задающий роль ИИ, технологический стек и правила валидации.
4. **Синхронизация локальных файлов**: Рекомендуется использовать специальный скрипт для сборки актуального контекста или загружать ключевые файлы конфигурации (`CLAUDE.md`, `todo.md`).

**Оптимальная структура папки `.claude/`**:
- `.claude/config.json` — конфигурация безопасности и ограничений выполнения команд ИИ.
- `.claude/custom_instructions.md` — файл с правилами и промптами, копируемый в интерфейс Claude Projects.
- `.claude/project_context.md` — компилируемый лог структуры проекта и файлов для быстрой загрузки.

---

## 🤖 Раздел 20: Примеры ролей агентов в vibecoding роях (L2_23)

При работе в парадигме роя ИИ-агентов крайне важно разделять обязанности и наделять каждую сессию ИИ четко выраженной ролью. Это предотвращает распыление внимания модели и улучшает качество генерируемого кода.

### Шаблоны ролей агентов

1. **iOS Developer (Swift & SwiftUI Expert)**:
   ```markdown
   Роль: Senior iOS Engineer
   Стек: Swift, SwiftUI, Combine/Swift Concurrency, Clean Architecture
   Правила:
   - Писать декларативный, типобезопасный код.
   - Использовать строгие паттерны Swift Concurrency (async/await, Actors).
   - Избегать UIKit, если нет явного требования.
   - Обязательно писать юнит-тесты на бизнес-логику (XCTest).
   ```

2. **Code Reviewer (Strict Anti-Slop Analyst)**:
   ```markdown
   Роль: Lead Code Reviewer & Quality Gatekeeper
   Задача: Анализировать предложенный код на предмет архитектурного мусора («slop»), лишнего дублирования и логических ошибок.
   Правила:
   - Требовать строгую обработку ошибок (без пустых catch блоков).
   - Проверять валидацию nullable/optional типов.
   - Убеждаться, что размер одного файла не превышает 200 строк.
   - Использовать строгие критерии DoD.
   ```

3. **UI/UX Designer (Tailwind & Premium CSS Artist)**:
   ```markdown
   Роль: Premium Frontend & CSS Architect
   Стек: HTML5, CSS Custom Properties, Vanilla JS, Glassmorphism, CSS Animations
   Правила:
   - Использовать изысканные цветовые палитры (никаких чистых красных/зеленых/синих цветов).
   - Реализовывать микро-анимации при наведении (hover) и переходах (transitions).
   - Гарантировать полную адаптивность и отзывчивость интерфейса.
   - Следовать гайдлайнам Apple Human Interface Guidelines / Material Design.
   ```

4. **Mentor / Debugging Assistant**:
   ```markdown
   Роль: Technical Mentor & Debugger
   Задача: Помогать разработчику находить сложные баги и понимать принципы работы систем.
   Правила:
   - Не выдавать готовый код сразу, если не попросят.
   - Сначала объяснить причину бага и физику процесса.
   - Задавать наводящие вопросы для лучшего понимания.
   ```

---

## 🔄 Раздел 21: Паттерн "Context Reset" — регулярная гигиена сессий (L2_24)

При длительной беседе с ИИ неизбежно происходит деградация контекста: накапливаются мелкие ошибки, ИИ начинает путаться в файлах, забывает первоначальные инструкции и генерирует менее точные ответы. 

**Паттерн "Context Reset" (Регулярный сброс сессий)** — это систематическое обнуление чата с переносом актуального состояния разработки.

### Алгоритм сброса контекста:
1. **Фиксация состояния (Handoff)**: Запишите текущий статус разработки, список выполненных задач и следующие 3 приоритетных шага в файл `todo.md` или `handoff.md`.
2. **Создание новой сессии**: Закройте текущий чат и откройте абсолютно новый, чистый диалог.
3. **Инициализация payload'а**: В первом же сообщении загрузите файлы `CLAUDE.md`, `todo.md` и схему БД (если применимо).
4. **Формулировка стартовой задачи**: Передайте ИИ четкое указание продолжить работу с зафиксированной точки.

Это снижает потребление токенов до 80%, убирает галлюцинации и гарантирует высокую скорость генерации кода.

---

## 🧠 Раздел 22: Режимы глубокого мышления и Thinking Budget (L2_25)

Современные модели рассуждения (Claude 3.7 Sonnet, Gemini Pro, o1/o3-mini) обладают встроенным блоком логического планирования (Reasoning/Thinking). Понимание того, как управлять временем и бюджетом мышления модели, критично для решения сложных алгоритмических задач.

### Управление бюджетом мышления (Thinking Budget & Reasoning):
1. **Когда отключать/снижать мышление**:
   - Простые рутинные задачи (написание HTML-структуры, добавление стилей, форматирование текста).
   - Небольшие рефакторинги, где логика очевидна.
   - Экономия токенов и ускорение ответа.
2. **Когда форсировать максимальный уровень мышления**:
   - Проектирование архитектуры базы данных и связей.
   - Реализация сложных алгоритмов синхронизации, транзакций или работы с памятью.
   - Поиск трудновоспроизводимых багов, утечек памяти или race conditions.

**Текстовые триггеры для глубокого мышления**:
Чтобы заставить модель рассуждать максимально детально перед написанием кода, используйте явные инструкции:
> *"Сделай глубокий пошаговый анализ (Reasoning) текущей структуры. Не пиши код, пока не распишешь все потенциальные граничные случаи (edge cases), проблемы конкурентности и влияние на производительность. Выдели на этап планирования не менее 80% бюджета токенов."*

---

## 📝 Раздел 23: Шаблоны промптов для Claude Projects (L2_26)

Шаблонизация запросов гарантирует воспроизводимость результатов разработки и жесткое соблюдение стандартов качества кодовой базы.

### 1. Системный промпт (System Prompt / Custom Instructions)
```markdown
Ты — Antigravity, элитный ИИ-ассистент по парному программированию.
Стек проекта: Vanilla HTML, CSS Custom Properties, ES6 JS.
Правила оформления кода:
1. Писать чистый, модульный код без внешних тяжелых зависимостей.
2. Использовать премиальную темную тему, эффект Glassmorphism, мягкие градиенты и микро-анимации.
3. Все интерактивные элементы должны быть полностью функциональны в DOM (hover-эффекты, плавное открытие).
4. Запрещено использовать заглушки (placeholders). Если нужна графика — генерировать SVG.
5. Не стирать существующие комментарии и логику, если это не требуется по задаче.
```

### 2. Мета-промпт планирования (Meta-Prompt)
```markdown
Изучи предложенную задачу: [ОПИСАНИЕ]. 
Перед написанием кода сформируй технический план реализации в формате Markdown.
План должен содержать:
1. Архитектурные изменения (какие файлы изменятся, какие будут созданы).
2. Описание логики работы и обработки ошибок.
3. План тестирования (какие тест-кейсы нужно проверить).
Не переходи к написанию кода, пока я не утвержу этот план.
```

### 3. Задачный промпт (Task-Prompt)
```markdown
Реализуй шаг №[X] из утвержденного плана: [ДЕТАЛИ ШАГА].
Требования:
- Напиши чистый код для модуля [ИМЯ_МОДУЛЯ].
- Предусмотри обработку ошибок и логирование в консоль.
- Обнови файл handoff.md, указав статус выполнения.
Выведи измененные фрагменты кода в формате diff-блоков.
```
