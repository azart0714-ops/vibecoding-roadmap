...
console.log("=== ЗАПУСК ВАЛИДАЦИОННЫХ ТЕСТОВ СЕМАНТИКИ (CYCLE 5) ===");
...
// 2. Validate the 6 new/enhanced L2 nodes presence and structure
const targetIds = [
  'l2_21_five_critical_mistakes',
  'l2_22_claude_projects',
  'l2_23_agent_roles',
  'l2_24_context_reset',
  'l2_25_thinking_budget',
  'l2_26_claude_prompt_templates'
];
...
targetIds.forEach(id => {
  ...
  if (node.level !== 'L2') {
    console.error(`❌ Ошибка: Узел ${id} имеет неверный уровень: ${node.level} (ожидался L2)`);
    failed = true;
  }
...
if (failed) {
  ...
  console.log("⭐️ ВСЕ СЕМАНТИЧЕСКИЕ ТЕСТЫ CYCLE 5 УСПЕШНО ПРОЙДЕНЫ!");
  process.exit(0);
}
